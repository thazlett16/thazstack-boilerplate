...
import { RangeCalendar } from '#src/components/ui/range-calendar';
...
export function DateRangePicker<T extends DateValue>({
  ...
  return (
    <AriaDateRangePicker
      {...props}
      className={composeTailwindRenderProps(props.className, 'group flex flex-col gap-1 font-sans max-w-full')}
    >
      ...
      <FieldGroup className="min-w-[208px] w-auto cursor-text disabled:cursor-default">
        <div className="flex-1 w-fit flex items-center overflow-x-auto overflow-y-clip [scrollbar-width:none]">
          ...
          <DateInput
            slot="end"
            className="flex-1 ps-2 pe-3 text-sm"
          />
        </div>
