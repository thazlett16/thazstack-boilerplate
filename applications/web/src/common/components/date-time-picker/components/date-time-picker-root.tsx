...
export interface DateTimePickerRootProps<T extends DateValue> extends RACDatePickerProps<T> {
  label?: ReactNode;
  description?: ReactNode;
}

export function DateTimePickerRoot<T extends DateValue>({
  label,
  description,
  granularity = 'minute',
  ...props
}: DateTimePickerRootProps<T>) {
  ...
  return (
    ...
    >
      ...
      <RACPopover className={slots.popover()}>
        <Dialog>
          <Calendar />
          {/* Time selector below the calendar */}
